...
async function loadProfile() {
  try {
    ...
    if (user.election_card_image) {
      document.getElementById('card-preview').innerHTML = `<img src="${user.election_card_image}" alt="Election Card" class="profile__card-img">`;
    }
...
async function uploadCard(e) {
  ...
  try {
    ...
    document.getElementById('card-preview').innerHTML = `<img src="${data.imagePath}" alt="Election Card" class="profile__card-img">`;
...
async function loadChatSessions() {
  try {
    ...
    if (!sessions.length) { container.innerHTML = '<p class="profile__empty">No chat history yet. Start a conversation!</p>'; return; }

    container.innerHTML = sessions.map(s => {
      const started = new Date(s.started).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' });
...
async function loadVotingPlans() {
  try {
    ...
    if (!plans.length) { container.innerHTML = '<p class="profile__empty">No saved voting plans. Create one in the Voting Plan tab!</p>'; return; }
