...
async function startQuiz() {
  ...
  startEl.innerHTML = '<div style="text-align:center;padding:48px"><div class="spinner"></div><p style="margin-top:16px;color:var(--text-secondary)">Generating questions...</p></div>';
  ...
  try {
    ...
    startEl.innerHTML = '<div style="text-align:center;padding:48px"><div class="quiz__start-icon">🧠</div><h3>Ready to test your election knowledge?</h3><p style="color:var(--error);margin:12px 0">Failed to generate quiz. Check your API key.</p><button class="btn btn--primary btn--lg" id="quiz-begin">Try Again</button></div>';
    document.getElementById('quiz-begin')?.addEventListener('click', startQuiz);
...
function renderQuestion() {
  ...
  optionsEl.innerHTML = q.options.map((opt, i) =>
    `<div class="quiz__option" data-index="${i}">${opt}</div>`
  ).join('');
...
function selectAnswer(index) {
  ...
  // Disable all
  options.forEach(o => { o.classList.add('quiz__option--disabled'); o.style.pointerEvents = 'none'; });
...
function showResults() {
  ...
  let emoji = '🎉', msg = 'Outstanding! You\'re an election expert!';
  if (pct < 40) { emoji = '📚'; msg = 'Keep learning! Democracy needs informed citizens.'; }
  else if (pct < 70) { emoji = '👍'; msg = 'Good effort! A little more learning and you\'ll be an expert.'; }
  else if (pct < 100) { emoji = '🌟'; msg = 'Great job! You know your elections well!'; }
