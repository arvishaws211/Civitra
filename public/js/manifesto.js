...
function renderParties(data) {
  ...
  let html = '<div class="manifesto__group"><h4>National Parties</h4><div class="manifesto__chips">';
...
function updateCompareBtn() {
  ...
  btn.textContent = selectedParties.length < 2
...
async function runComparison() {
  ...
  content.innerHTML = '<div style="text-align:center;padding:60px 20px"><div class="spinner"></div><p style="margin-top:16px;color:var(--text-secondary)">Analyzing manifestos with AI...<br><small>This may take a moment</small></p></div>';
  ...
  try {
    ...
    content.innerHTML = '<p style="color:var(--error);text-align:center;padding:40px">Failed to generate comparison. Please try again.</p>';
...
function renderComparison(data) {
  ...
  if (data.rawContent) {
    try {
      const cleaned = data.rawContent.replace(/```json\s*/gi, '').replace(/```\s*/g, '').trim();
