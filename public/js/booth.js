...
function displayBooths(places) {
  ...
  html += '</div>';
  html += '<p class="booth__disclaimer-note">These are potential booth locations based on nearby public buildings. Always verify your assigned booth via the ECI portal.</p>';
  ...
  resultsEl.querySelectorAll('.booth__item-dir').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      window.open(`https://www.google.com/maps/dir/?api=1&origin=${userPosition.lat},${userPosition.lng}&destination=${lat},${lng}&travelmode=driving`, '_blank');
    });
  });
}
